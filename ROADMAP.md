...
**进度（2026-07-25 §F5d ext 工具/命令 host wiring + 真卸载 two-phase drop——§F5 续作收尾：ext 工具+slash 命令 wire 进 host per-turn `ToolRegistry` + safe unload[`clear_tools`/`clear_commands` on reload + two-phase `Library` drop]，`feat/ext-wire-unload`）：**

接 §F5c（INSTALL 侧）。§F5c 把 ext 工具/命令的 host wiring 留作 deferred（`bound_tools()` 仅 display[`extension_commands.rs:170` len]+verification[`loader.rs:117`/`installer.rs:230`] 调用，NOT wired 进 host per-turn `ToolRegistry`；`try_dispatch_command` defined-but-never-called[async, dead]；`ExtensionToolSpecAdapter` test-only-constructed[`extension.rs:157,166`]）+ 真卸载留作 Q1 bounded 留存保底（`libraries: Mutex<Vec<Library>>` unload-only，reload 不清——保底正确性）。§F5d 落地两者：(a) wire ext 工具+slash 命令进 host per-turn `ToolRegistry`（make them callable）；(b) safe unload——`clear_tools`/`clear_commands` on reload（mirror `clear_handlers`）+ two-phase `Library` drop（UI-thread drain→`pending_drop`，engine op-loop top `drop_pending` at turn-boundary safe moment）。spec：`docs/superpowers/specs/2026-07-24-codesmith-extension-system-slice-5d-design.md`。

**关键设计决策 + API reconciliations：**
- **R-T1 工具 wire**：`bound_tools()` 有 display+verification 调用但 NOT wired 进 host per-turn `ToolRegistry` → 现 production-wired via `register_extension_tools`[`agent-runtime/tools/extension.rs:91`，wraps each bound tool in `ExtensionToolSpecAdapter`→`ToolRegistry::register`，fail-closed last-wins cache-invalidate]→`EngineHost::build_turn_dispatcher`[plugin-tools 后/catalog 前]。`ExtensionToolSpecAdapter` was test-only-constructed→现 production-wired。`bound_context()`[`runner.rs:397`] 新 accessor[upcast `Arc<dyn ExtensionCommandContext>`→`Arc<dyn ExtensionContext>`，Rust 1.86+ trait-upcasting]。Per-turn rebuild→clearing `runner.tools[id]` before next turn 充分（no persistent host holder）。
- **R-T2 命令 wire**：`ExtensionRunner::try_dispatch_command`[`runner.rs:370` async] was defined-but-never-called[dead]→现 production-wired via `try_dispatch_extension_command`[`extension_commands.rs:67`] in `commands::execute`[`mod.rs:597`，`/extension` meta-tier 后/static match 前]。sync→async bridge via `std::thread::scope`+spawned current-thread rt[mirror `populate_extension_runtime` `engine.rs:429`]；spec-literal `block_on` form rejected——would panic-on-shutdown on TUI's multi-thread `#[tokio::main]`[tokio blocking/shutdown.rs]。`None` on no-runner/no-match→built-ins 仍 run。
- **R-T3 clear trio + ordering**：`ExtensionRunner` had lone `clear_handlers`[`runner.rs:171`]→现 trio `clear_handlers`/`clear_tools`[`runner.rs:184`]/`clear_commands`[`runner.rs:191`][mirror `clear_handlers`，`Mutex<HashMap>.clear()`] + the (T4) `drain_libraries_to_pending`[`runner.rs:207`]。`reload_extension_runtime`[`engine.rs:484`] ordering：clear_handlers→clear_tools→clear_commands→drain_libraries_to_pending→invalidate→populate[discover→reconcile→load→bind_core]。Safe concurrent w/ in-flight turn[tools/commands refcounted Arc+name-keyed，per-turn `ToolRegistry` captured own Arc→clear only affects FUTURE turns' rebuild，`Library` alive until T4 drop]。
- **R-T4 two-phase Library drop**：`libraries` was `Mutex<Vec<Library>>` unload-only[Q1 bounded leak]→现 two-phase drain→pending→drop safe。`pending_drop: Mutex<Vec<Library>>`[`runner.rs:126`] field + `drain_libraries_to_pending`[UI-thread MOVE：`mem::take` libraries→pending_drop under one lock，`Library` stays alive] called from `reload_extension_runtime`[`engine.rs:502`] + `drop_pending`[engine op-loop top `mod.rs:527` before `match op`：`mem::take` pending→drop，the safe moment main-thread `HostAgentExecutor`[the only in-flight dylib `Arc` holder] already dropped between turns `host_executor.rs:1850`]。Makes `/extension reload`+uninstall safe concurrent w/ in-flight turns。
- **§4b reconciliation（CRITICAL—from T4 spec review）**：spec FILE's §4b literally suggested "add a provenance marker on host-registry entries（or force-subset-when-dylib-loaded）" for the `inherit_full_registry=true` sub-case。**This was resolved DIFFERENTLY + more simply**：§4b is structural——`SubAgentRuntime`[`subagent/mod.rs:548-608`] has NO `extension_runner` field + `SubAgentToolRegistry::new`[`subagent/mod.rs:6111`] rebuilds its OWN fresh built-in `ToolRegistry`[`ToolRegistryBuilder::new().with_full_agent_surface(...)` `:6160`，no parent `Arc` clone]→ext tools[added only in `build_turn_dispatcher`] never reach a subagent's effective set，regardless of `inherit_full_registry`。NO provenance marker / force-subset / runtime subagent-check guard was added。spec file §4b last-paragraph + §7 + §10 are now STALE on this point——the structural resolution supersedes the spec's provenance-marker suggestion（spec file can be reconciled later）。Regression test `subagent_ext_tool_excluded_from_effective_set`[`subagent/tests.rs:941`] locks the structural exclusion[expected Green，both inherit settings]。
- **§5 no-provenance simplification stands**：clear-all-then-re-load-present makes per-extension ownership tracking unnecessary[reload re-derives live set from disk，uninstalled ext simply isn't re-loaded]。

**落地步骤：**
1. T1 wire ext tools into `EngineHost::build_turn_dispatcher`[`register_extension_tools` helper in `agent-runtime/tools/extension.rs:91`]+`bound_context()` accessor[trait-upcast]。agent-runtime 1163→1165[+2]。
2. T2 wire ext slash commands into `commands::execute`[`try_dispatch_extension_command` helper in `extension_commands.rs:67`，`CommandOutput`→`CommandResult`；`std::thread::scope`+current-thread rt bridge，NOT spec-literal `block_on`；tier after `/extension` meta-tier before static match]。tui 2835→2866[T2 +4 dispatch tests incl. 2 added per code-quality nits #2a/#2b：`try_dispatch_extension_command_resolves_contributed_command`+None-fall-through{no-runner,unknown-command}+SendMessage→`AppAction::SendMessage` arm]。
3. T3 reload-deferred `clear_tools`/`clear_commands`[mirror `clear_handlers`，`Mutex<HashMap>.clear()`]+`reload_extension_runtime` ordering[clear→drain→invalidate→populate]。ext 48→49[+1 `clear_tools_and_clear_commands_empty_registries` Red→Green test；test helper `runner_with_fixture_dylib` direct `load_dylib` on env! `CODESMITH_FIXTURE_DYLIB`]。
4. T4 two-phase `Library` drop[`pending_drop:Mutex<Vec<Library>>` field + `drain_libraries_to_pending` UI-thread MOVE + `drop_pending` engine op-loop-top turn-boundary DROP]+§4b subagent exclusion regression test[structural-automatic，no guard]。ext 49→50[+1 drain test]；tui +1 §4b test[`subagent_ext_tool_excluded_from_effective_set` expected Green both inherit settings]。
5. T5 docs：`/extension uninstall` message rewrite[drop §F5b-Q1 bounded-retention caveat→§F5d live bindings clear on next `/extension reload`+dylib unloads safely at next turn boundary op-loop `drop_pending`]；EXTENSIONS.md[intro tools+commands wired-live per-turn §F5d T1+T2 + In-TUI reload row clear trio+drain + uninstall row ✅§F5c→refined §F5d messaging + Sandbox Stance ext-tools-main-turn-only NOT inherited by subagents §4b structural + two-phase `Library` drop §4a + Miri note]；ROADMAP §F5d progress block。docs+message-only——ext/agent/agent-runtime counts unchanged by T5。

**测试/验证：** `cargo build --workspace` 全绿；`codesmith-extensions --lib` 48→50[T3 +1 clear test，T4 +1 drain test；T5 docs-only 不变]；`codesmith-agent --lib` 98（不变——§F5d 不触 agent）；`codesmith-agent-runtime --lib` 1163→1165[+2 T1]；`codesmith-tui --bin codesmith-tui` 2835→2866[T2 +4 dispatch tests incl. 2 per code-quality nits，T4 +1 §4b test；baseline 26 pre-existing runtime_api env-fail noted，in THIS env HTTP-server binds→0 fail；T5 docs+message-only，grep 确认无 test 断言旧 uninstall 消息——count 不变]。grep：`register_extension_tools` in agent-runtime ≥1、`try_dispatch_extension_command` in tui ≥1、`clear_tools`/`clear_commands`/`drain_libraries_to_pending`/`drop_pending` in extensions ≥1 each、`drop_pending` in engine op-loop `mod.rs:527` =1、`pending_drop` field on `ExtensionRunner` =1、`subagent_ext_tool_excluded_from_effective_set` in tui =1；§F5b/§F5c 不变项（`libloading`/`toml` in extensions `Cargo.toml`、`loader.rs`/`manifest.rs`/`build.rs` 存在、`discover_dylib` in `engine.rs` ≥1、`codesmith_register_extension` in fixture ≥1、`TrustReason::FirstLoad` in tui=1）。

**By-design gaps（显式 out-of-scope，rationale 已记录）：**
- §4b subagent-exclusion is structural-automatic（no guard）——by design[structural resolution supersedes spec file's provenance-marker suggestion；spec §4b last-paragraph + §7 + §10 stale on this point]。
- two-phase drop's UB-safety is proven by invariant + single-call-site discipline，NOT a Miri run[dylib+Miri unreliable——libloading's `Library::drop` runs `dlclose`/`FreeLibrary` which Miri doesn't model]——by design[the invariant is the proof]。
- `CratesIo`/`Prebuilt` source impls[command `install_precheck` 早返回 "§F5c-later"]——§F5c deferred，unchanged by §F5d。
- tui-level dylib e2e[`run_tui` 触发 wire/reload/unload]：§F5 precedent[App fixture 比例失衡]；机制由 ext-crate 单测+§F5d regression tests 覆盖。

**下一聚焦工作：**
- 残项（按需）：P2 doc drift + §E4 follow-up + `CratesIo`/`Prebuilt` impl + spec file §4b/§7/§10 reconciliation[stale on provenance-marker suggestion，superseded by structural resolution]——均 on-demand / 非阻塞。
...
Library unload (Q1 accepts bounded retention; **shipped §F5d** as a safe two-phase drop — see the §F5d block). tui dylib e2e deferred per §F5
