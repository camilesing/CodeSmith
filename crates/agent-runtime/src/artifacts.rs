...
//
// Physically migrated from `codesmith-tui`. The TUI keeps a glob re-export
// shim (`pub use codesmith_agent_runtime::artifacts::*;`) so historical
// `crate::artifacts::<item>` paths keep resolving.
//
// Test-session-root override helpers (`set_test_artifact_sessions_root`,
// `TEST_ARTIFACT_SESSIONS_GUARD`) are compiled unconditionally and exposed
// `pub` (not `cfg(test)`-gated) so dependent crates' test builds can reach
// them — dependencies are never built with `cfg(test)` (see
// `crate::test_support` for the same convention). They are only ever invoked
// from test contexts.
...
pub static TEST_ARTIFACT_SESSIONS_GUARD: std::sync::Mutex<()> = std::sync::Mutex::new(());
