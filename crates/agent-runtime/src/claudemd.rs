...
//! Project (`{cwd}/AGENTS.md`/`CLAUDE.md`/…), Local
...
use crate::project_context::{
    GLOBAL_AGENTS_RELATIVE_PATH, GLOBAL_AGENTS_VENDOR_NEUTRAL_PATH, PROJECT_CONTEXT_FILES,
    load_context_file,
};
...
pub enum MemoryTier {
    /// Organisation-managed policy (`/etc/codesmith/CLAUDE.md`).
    Managed,
    /// User-wide preferences (`~/.codesmith/AGENTS.md`, …).
    User,
    /// Project-local instructions (`{cwd}/AGENTS.md`, `CLAUDE.md`,
...
/// Managed tier: `/etc/codesmith/CLAUDE.md`, mirroring the
/// `default_managed_config_path` convention. Absent on platforms without `/etc`.
...
pub fn load_all_memory_tiers(workspace: &Path, home: Option<&Path>, excludes: &[String]) -> String {
    ...
    if let Some(home) = home
        && let Some(user) = load_user_tier(home, &exclude_paths, &mut processed)
    {
        blocks.push(user);
    }
