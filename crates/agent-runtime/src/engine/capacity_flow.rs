...
/// Drop kept messages whose `tool_result` blocks reference a `tool_use` that a
/// front-trim removed. A front-trim can only orphan *results* (calls precede
/// their results in the transcript), and dropping only ever shrinks the
/// transcript — preserving the caller's token-budget guarantee (unlike the
/// compaction planner's `enforce_tool_call_pairs`, which grows a pin set).
/// Cascades until stable in case a dropped message also carried calls that
/// other results depend on. Returns how many additional messages were
/// removed.
pub(crate) fn enforce_tool_pairs_after_front_trim(kept: &mut Vec<Message>) -> usize {
    let mut removed = 0usize;
    loop {
        // Owned ids: the set must outlive the `retain`'s mutable borrow.
        let live_calls: std::collections::HashSet<String> = kept
            .iter()
        ...
        kept.retain(|m| {
            let orphaned = m.content.iter().any(|b| match b {
                ContentBlock::ToolResult { tool_use_id, .. } => !live_calls.contains(tool_use_id),
                _ => false,
            });
            !orphaned
        });
    ...
    for msg in messages.iter().rev() {
        ...
        for block in msg.content.iter().rev() {
            if let ContentBlock::ToolUse {
                ...
                && let Some((content, is_error)) = results.get(id.as_str())
                && *is_error != Some(true)
...
impl CapacityGateProbe {
    ...
    fn build_observation(
        ...
        let context_window = usize::try_from(
            context_window_for_model(&self.model).unwrap_or(LEGACY_MODEL_CONTEXT_WINDOW_TOKENS),
        )
        .unwrap_or(usize::try_from(LEGACY_MODEL_CONTEXT_WINDOW_TOKENS).unwrap_or(128_000))
        .max(1);
    ...
    pub fn capacity_observation(&self, turn: &TurnContext) -> CapacityObservationInput {
        ...
        let context_window = usize::try_from(
            context_window_for_model(&self.session.model)
                .unwrap_or(LEGACY_MODEL_CONTEXT_WINDOW_TOKENS),
        )
        .unwrap_or(usize::try_from(LEGACY_MODEL_CONTEXT_WINDOW_TOKENS).unwrap_or(128_000))
        .max(1);
...
mod front_trim_tests {
    ...
    fn front_trim_drops_orphaned_results() {
        // A front-trim removed the call for A but kept its result: the
        // orphaned result must be dropped or the next request violates the
        // every-tool_use-has-a-tool_result contract.
        let mut kept = vec![
            tool_result("A"),
            assistant_call("B"),
            tool_result("B"),
        ...
        assert!(kept.iter().all(|m| !m.content.iter().any(
    ...
    fn intact_pairs_survive() {
        let mut kept = vec![assistant_call("X"), tool_result("X"), user_text("tail")];
        let removed = enforce_tool_pairs_after_front_trim(&mut kept);
        assert_eq!(removed, 0);
        assert_eq!(kept.len(), 3);
    }
    ...
    #[test]
    fn drops_cascade_when_a_result_message_also_carried_a_call() {
        // Message order after a trim: [result(A) + call(B) combined, result(B)].
        // Dropping the orphaned result(A) message must cascade — result(B)
        // loses its call and goes too, leaving only the trailing text.
