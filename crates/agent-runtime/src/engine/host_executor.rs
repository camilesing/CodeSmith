...
//! It replaced the retired legacy turn handler (~2.4k lines) —
//! `Engine::handle_send_message` now routes through [`HostAgentExecutor`],
//! and the legacy handler is deleted. Provenance comments below that
//! attribute behavior to that retired fn; its full ~2.4k-line body is
//! viewable via `git show ab4f4fc5:crates/agent-runtime/src/engine/turn_loop.rs`
//! in the pre-fork history.
//!
...
//!    the retired turn handler). This was the proof that local-state guardrails need no
...
//!    the retired turn handler's outer "stream died with nothing" retry
//!    (the retired turn handler). A stream that dies *after* content was
...
//!    two-stage shrink mirroring the retired turn handler's pre-request
//!    compaction (the retired turn handler): (a) **micro-compaction** — if the
...
//!    (mirrors the retired turn handler). The recovery cascade runs
...
//!    the retired turn handler's `early_tool_start_safe` + `early_tool_tasks`
//!    map. The tool loop
...
//!    mirroring the retired turn handler's non-blocking completion drain.
//!    The executor
...
//! the live production turn path; the retired turn handler is deleted. The
...
//!    at retired the retired turn handler which folds `session.compaction_summary_prompt`
...
//!
//! The "known gaps in the LSP flush" inventory moved with the collect /
//! flush pair to the `engine/turn/postprocess.rs` module docs.
//!
...
//!   `tool_uses.is_empty()` tail (the retired turn handler) also ran two
...
//!   tool-call assistant message (the retired turn handler). The executor
...
//!     the retired turn handler. No-op when `reinject` is `None` (no probe ⇒ no
...
//!   context-length error (the retired turn handler). This executor now does the
...
//!   recovery budget (mirrors the retired turn handler). The budget is bounded by
...
//!   select!` (the retired turn handler) over cancel / completion `recv().await`
...
//!   `config.trust_mode` (the retired turn handler). `ChatHistory` exposes no
...
//!   declares `pending_steers` before the stream loop (the retired turn handler),
...
/// waiting for completion sentinels. Bounds how long the turn can stay parked
/// when a child dies (panics or is aborted) without emitting its sentinel —
/// see the hold's re-check loop in [`HostAgentExecutor::run_inner`].
...
use super::turn::{
    EarlyToolTask, StreamRoundOutcome, approval_intent_summary, should_emit_thinking_only_status,
    should_hold_turn_for_subagents, subagent_completion_runtime_message,
};
...
/// executor ⇒ LSP disabled for this run (collect + flush are no-ops). The
/// collect / flush impls live in `turn::postprocess` (codebase-health §1
/// split), which is why the fields are `pub(crate)`.
pub struct LspProbe {
    pub(crate) manager: Arc<dyn LspManagerApi>,
    /// Workspace root for relativizing edited paths (mirrors
    /// `session.workspace`, which `ChatHistory` does not expose).
    pub(crate) workspace: PathBuf,
    pub(crate) pending: Arc<std::sync::Mutex<Vec<DiagnosticBlock>>>,
}
...
/// the retired turn handler's loop).
///
...
pub struct ReinjectProbe {
    ...
    /// `recent_read_files`. Mirrors the retired the retired turn handler's call
...
/// matching the production per-turn counter (the retired turn handler). The forced
...
/// the retired turn handler push sites):
...
enum CapacityPreflight {
    ...
    /// the retired turn handler `continue`).
    RetryStep,
...
pub struct HostAgentExecutor {
    ...
    /// The collect / flush impls live in `turn::postprocess` (codebase-health
    ...
    /// resumes — mirroring the retired turn handler's non-blocking completion
    /// drain (the retired turn handler). The **blocking hold** for
    /// still-running children (`should_hold_turn_for_subagents` + a `biased
    /// select!` over cancel / completion `recv().await` / steer `recv().await`)
    ...
    /// the retired turn handler's seven cancel checkpoints: (A) loop-top gate,
    ...
    /// production's `registry.approval_requirement_for(..)` (the retired turn handler).
    ...
    /// matching the retired production the retired turn handler push sites.
    ...
    /// value wins, mirroring the retired the retired turn handler);
    /// `run_inner` adds each completed stream's usage here (mirrors
    /// the retired turn handler's `turn.add_usage(&usage)`). The host
...
impl HostAgentExecutor {
    ...
    /// the retired turn handler (`turn.add_usage(&usage)`) used to do inline. The
    ...
    /// `Engine::refresh_system_prompt` at the retired the retired turn handler,
    ...
    /// (retired the retired turn handler, "Ensure system prompt is up to date with
    ...
    /// a concise reminder. Mirrors the retired the retired turn handler:
    ///
    /// ```text
    ...
    /// the retired turn handler's top-of-loop compaction
    /// (the retired turn handler): a cheap no-API micro-compact pass (clear old
    ...
    async fn run_compaction(&self, client: &LlmClientHandle, history: &mut dyn ChatHistory) {
        ...
        // the retired turn handler (`session.circuit_breaker.should_attempt()`).
        {
        ...
        // (mirrors the retired turn handler) BEFORE calling `compact_messages_safe`:
    ...
    /// trim). Mirrors the retired turn handler's Gate B
    /// (the retired turn handler) — the always-on hard token-budget preflight,
...
impl HostAgentExecutor {
    ...
    async fn run_inner<'a>(
        ...
        // `LoopGuard` per turn, matching the retired turn handler).
        ...
        // the retired turn handler). Persists across steps within one run;
        ...
        // within one run (mirrors the retired turn handler). Persists across steps
        ...
        // the retired turn handler. Calling it inside `run_inner` would discard
        // steers the host queued for THIS turn before calling `run`. It is a
        // `pub` method the host calls before `run` at the wire-in step.
        //
        // `'step` — labeled so the sub-agent hold's steer arm can `continue`
        // this loop from inside its own inner re-check loop.
        'step: loop {
            // Checkpoint A — loop-top cancel gate (mirrors
            // the retired turn handler). First thing every iteration: if the
            ...
            // the retired turn handler) — before compaction, the LSP flush, and the
            ...
            // the retired turn handler, "Ensure system prompt is up to date with
            ...
            // Auto-compaction mirrors the retired turn handler (steer →
            ...
            if let Some(gate) = &self.capacity_gate
                && let Some(snapshot) = gate.observe_pre_turn(
                    history.messages(),
                    step,
                    ...
                    *self
                    ...
                    if decision.action == GuardrailAction::TargetedContextRefresh {
                        ...
                        *self
                            ...
                            .lock()
                            .expect("pending_targeted_refresh_outcome mutex poisoned") = outcome;
                    }
                }
            }
            ...
            // that arrive during streaming (mirrors the retired turn handler declaring
            ...
            // count + categories (mirrors the retired turn handler). Categorized
                ...
                .await
            {
                Ok(StreamRoundOutcome::Content {
                    ...
                    // end-of-stream handoff the retired the retired turn handler
            ...
            // retired the retired turn handler deferred-decide). Keep thinking
            ...
            // (the retired turn handler). slice 39 §E.
            ...
            if tool_uses.is_empty() {
                // Mid-stream steer flush (mirrors the retired turn handler):
                ...
                // Mirrors the retired turn handler's non-blocking completion drain
                ...
                if let Some(probe) = &self.subagent {
                    ...
                    // Blocking hold (mirrors the retired turn handler): when
                        ...
                        && let Some(api) = &self.subagent_api
                    {
                        ...
                        if should_hold_turn_for_subagents(0, running) {
                            // Re-check loop (P1 review fix): a child that
                            // panicked or was aborted never emits its
                            // completion sentinel, and `running_count()` can
                            // drop to 0 after this check — without the
                            // timeout arm the turn would park in
                            // `recv().await` forever ("Waiting on N
                            // sub-agent(s)" with nothing left to wait for).
                            // Clone the Arc so the lock guard borrows a
                            // local, leaving `self.steer` freely accessible
                            // in the steer arm (mirrors the approval race).
                            // The lock is uncontended (single consumer) and
                            // held across `recv().await` only inside the
                            // `select!` below.
                            ...
                            loop {
                                ...
                                // race (mirrors the retired turn handler).
                                // `biased` so cancel wins if both are ready. The
                                // cancel arm `return`s, the steer arm
                                // `continue 'step`s, the re-check arm
                                // `continue`s the inner loop — only the
                                // completion arm falls through to the
                                // `try_recv` drain below.
                                tokio::select! {
                                    biased;
                                    _ = async {
                                        match &cancel_token {
                                    ...
                                    Some(steer) = async {
                                        ...
                                        // Steer arm: inject the steered text as
                                        // a user message and resume the turn on
                                        // a fresh step (mirrors the retired
                                        // turn handler). Closes the "steer
                                        // post-stream resume" gap — steers that
                                        // arrive during the hold are surfaced
                                        // immediately rather than waiting for
                                        // the next step's pre-request drain.
                                        ...
                                        if still_running == 0 {
                                            tracing::warn!(
                                                "sub-agent hold re-check: no running \
                                                 sub-agents left but no completion \
                                                 sentinel arrived (child panicked or \
                                                 was aborted)"
                                            );
                                            break;
                                        }
                                ...
                                // Only reached if the completion arm won
                                // (cancel `return`ed, steer `continue 'step`ed,
                                // re-check `continue`d). The `recv()` future was
                                // consumed, releasing the borrow on
                                // `sub_guard` — drain any completions batched
                                // behind the first (mirrors the retired turn
                                // handler).
            ...
            // Mid-stream steer flush (mirrors the retired turn handler): push
            ...
            if let Some(gate) = &self.capacity_gate
                && let Some(snapshot) = gate.observe_post_tool(
                    history.messages(),
                    step,
                    &tool_call_ids_this_run,
                    system.as_ref(),
            ...
            // halt, mirroring the retired turn handler where cancel is
            ...
            if let Some(gate) = &self.capacity_gate
                && let Some(decision) = gate.decide_error_escalation(
                    history.messages(),
                    step,
                    &tool_call_ids_this_run,
                    system.as_ref(),
                ...
                )
            {
                ...
                *self
                    .pending_capacity_decision
                    .lock()
                    .expect("pending_capacity_decision mutex poisoned") = Some(decision.clone());
                self.emit_status(format!(
                    "Capacity: {} — {}",
                    decision.action.as_str(),
                    decision.reason
...
mod tests {
    ...
    //
    // The production the retired turn handler silently re-issues a request when
    ...
    // === steer (seam 1) ==================================================
    //
    // The production the retired turn handler drains queued steer inputs at the
    // very top of each step (the retired turn handler) — before the LLM request
    ...
    /// (retired the retired turn handler, refresh-before-compaction ⇒ the model sees a
    ...
    async fn thinking_only_turn_not_persisted_and_emits_status() {
        ...
        // seeded user message remains (mirrors the retired turn handler).
        assert_eq!(
            history.len(),
            1,
    ...
    // request snapshot picks up the compacted transcript (mirrors
    // the retired turn handler). `MockRound::StreamOpenErr` makes
    ...
    /// `try_recv` loop after `recv()` returns (mirrors the retired turn handler).
    ...
    /// simulates a child that panicked or was aborted right after the hold
    /// decision was made: no completion sentinel will ever arrive.
    ...
    #[async_trait::async_trait]
    impl SubAgentApi for VanishingSubAgentApi {
        async fn running_count(&self) -> usize {
            let n = self.calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            if n == 0 { 1 } else { 0 }
        }
        async fn list(&self) -> Vec<crate::subagent::SubAgentResult> {
            Vec::new()
        }
        async fn cleanup(&self, _max_age: std::time::Duration) {}
        async fn live_running_snapshots(&self) -> Vec<crate::subagent::SubAgentResult> {
            Vec::new()
        }
    }

    /// P1 review fix: the sub-agent blocking hold must not park forever when
    /// a child dies without emitting its completion sentinel. The sender is
    /// dropped (no sentinel ever), `running_count` is 1 at the hold decision
    /// and 0 on every re-check. Paused time auto-advances the re-check sleep,
    /// so a hang (the bug) makes the outer timeout fire and fail the test.
    #[tokio::test]
    async fn subagent_hold_exits_when_children_vanish_without_sentinel() {
        ...
        drop(sub_tx); // no completion sentinel will EVER arrive
        ...
        let executor = HostAgentExecutor::new(
            ...
            None,
            None,
            Some(Arc::new(tokio::sync::Mutex::new(sub_rx))),
            None,
            Some(Arc::new(VanishingSubAgentApi::new())),
        );
        // Real-time bound (no paused clock): the re-check interval is 5s,
        // so a healthy run finishes in ~5s; a hang exhausts 30s and fails.
        let outcome = tokio::time::timeout(
            std::time::Duration::from_secs(30),
            executor.run(&mut history, "do it".to_string()),
        )
        .await
        .expect("hold must not hang when children vanish without a sentinel");
        assert_eq!(outcome.expect("run"), StopReason::NoToolCalls);
        let _ = workspace_stamp;
    }
}
