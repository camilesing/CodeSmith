...
pub struct Engine {
    ...
    /// path on `Engine` is retired with the legacy turn loop).
    pub(crate) rx_approval: Arc<AsyncMutex<mpsc::Receiver<ApprovalDecision>>>,
    /// Write-only after the legacy turn-loop retirement: its consumer
    /// (`await_user_input`) retired with it; the executor owns its own user-input path.
    ...
    /// Orphaned by the legacy turn-loop retirement: it serialized tool calls
...
impl Engine {
    ...
    async fn handle_send_message(
        ...
        // The legacy engine turn loop is retired (deleted with its 3373-line body +
        ...
        // end-of-turn handoff the retired engine turn loop
    ...
    /// assistant message. Called from the executor turn loop before each API
