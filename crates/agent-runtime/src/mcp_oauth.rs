...
impl McpOAuthConfig {
    pub fn device_code_endpoint(&self) -> Result<&str> {
        if let Some(url) = self.device_code_url.as_deref().filter(|s| !s.trim().is_empty()) {
            return Ok(url);
        }
...
fn excerpt(body: &str) -> &str {
    let end = body.char_indices().map(|(i, _)| i).nth(200).unwrap_or(body.len());
    &body[..end]
}
...
pub async fn run_device_flow(
    ...
    println!();
    println!("Authorization required for MCP server `{server}` ({}).", config.provider);
    println!("  1. Open:  {}", code.verification_uri);
    println!("  2. Code:  {}   (expires in {}s)", code.user_code, code.expires_in);
    ...
    loop {
        ...
        match parse_token_response(&body) {
            ...
            DeviceTokenOutcome::Denied { error, description } => {
                bail!("authorization denied ({error}): {}", description.unwrap_or_default());
            }
        }
    }
}
...
mod tests {
    ...
    fn github_preset_fills_endpoints() {
        ...
        assert_eq!(config.device_code_endpoint().unwrap(), GITHUB_DEVICE_CODE_URL);
