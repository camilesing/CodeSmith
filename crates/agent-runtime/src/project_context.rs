...
//!
//! - `AGENTS.md` - Cross-agent project instructions (highest priority)
...
/// AGENTS.md is the cross-agent convention; CLAUDE.md provides Claude Code
/// compatibility. `.codesmith/` is the config directory.
...
fn load_global_agents_context(workspace: &Path, home_dir: Option<&Path>) -> Option<ProjectContext> {
    ...
    // Priority order:
    // 1. ~/.codesmith/AGENTS.md     (new config directory)
...
fn auto_generate_context(workspace: &Path) -> Option<String> {
    ...
    let content = format!(
        "# Project Structure (Auto-generated)\n\n\
         > This file was automatically generated by CodeSmith.\n\
         > You can edit or delete it at any time.\n\n\
         **Summary:** {summary}\n\n\
         **Tree:**\n```\n{tree}\n```"
    );

    // Create .codesmith/ directory
    if let Err(e) = std::fs::create_dir_all(&codesmith_dir) {
        tracing::warn!("Failed to create .codesmith/ directory: {e}");
        return None;
    }
...
This file provides guidance to AI agents (CodeSmith, Claude Code, etc.) when working with code in this repository.
...
mod tests {
    ...
    #[test]
    fn test_codesmith_specific_path_wins_over_agents_path() {
        let workspace = tempdir().expect("workspace tempdir");
        let home = tempdir().expect("home tempdir");

        let codesmith_dir = home.path().join(".codesmith");
        fs::create_dir(&codesmith_dir).expect("mkdir .codesmith");
        let codesmith_agents = codesmith_dir.join("AGENTS.md");
        fs::write(&codesmith_agents, "CodeSmith-specific instructions")
            .expect("write codesmith agents");
        ...
        assert!(
            instructions.contains("CodeSmith-specific instructions"),
            "CodeSmith-specific global file should win:\n{instructions}"
        );
        assert!(
            !instructions.contains("Vendor-neutral instructions"),
            "lower-priority .agents file should be skipped:\n{instructions}"
        );
        assert_eq!(ctx.source_path, Some(codesmith_agents));
    }
