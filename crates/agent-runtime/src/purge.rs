...
mod tests {
    ...
    // Minimal queue-driven `LlmClient` stub for the purge tests. The real
    ...
    // from a downstream crate). The purge pipeline only ever calls
    // `create_message`, so a tiny stub here is enough to keep these tests
