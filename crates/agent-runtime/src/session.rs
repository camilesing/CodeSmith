...
use crate::prompt_zones::{AppendLog, RebuildReason};
...
pub struct Session {
    ...
    /// Conversation history (API format) — an append-only [`AppendLog`].
    /// `push` is the only everyday mutation; wholesale replacement goes
    /// through [`Session::rebuild_transcript`] with a named
    /// [`RebuildReason`]. Slice reads work directly via `Deref`.
...
impl Session {
    ...
    /// Replace the whole transcript in one audited operation — the only
    /// sanctioned cache-busting path (compaction, overflow recovery,
    /// `/edit` rollback, session restore, cycle reseeds). The `reason`
    /// names the caller and is kept in the log's
    /// [`RebuildRecord`](crate::prompt_zones::RebuildRecord) for `/cache zones`.
    pub fn rebuild_transcript(&mut self, reason: RebuildReason, messages: Vec<Message>) {
        self.messages.rebuild(reason, messages);
    }
    ...
    /// borrowed by `SessionChatHistory`), mirroring the retired engine
    /// turn-loop observe site.
