...
pub const DEFAULT_REGISTRY_URL: &str =
    "https://raw.githubusercontent.com/camilesing/codesmith-skills/main/index.json";
...
impl InstallSource {
    ...
    pub fn parse(spec: &str) -> Result<Self> {
        ...
        if let Some(rest) = trimmed.strip_prefix("github:") {
            ...
            // case-insensitive owner/repo so `github:octocat/Foo` works.
...
mod tests {
    ...
    fn parse_github_source() {
        let s = InstallSource::parse("github:octocat/test-skill").unwrap();
        assert_eq!(
            s,
            InstallSource::GitHubRepo("octocat/test-skill".to_string())
        );
    }

    #[test]
    fn parse_github_source_rejects_missing_repo() {
        let err = InstallSource::parse("github:octocat").unwrap_err();
        assert!(err.to_string().contains("github source must"), "got: {err}");
    }

    #[test]
    fn parse_github_source_rejects_extra_slashes() {
        let err = InstallSource::parse("github:octocat/repo/extra").unwrap_err();
        assert!(err.to_string().contains("github source must"), "got: {err}");
    }
