...
//!
//! These helpers are compiled unconditionally (not `cfg(test)`-gated) so they
//! remain visible to dependent crates' test builds — dependencies are not
//! built with `cfg(test)`, so a `cfg(test)`-gated helper would be invisible
//! cross-crate. They are only ever *invoked* from test contexts, where env
//! mutation is serialised by [`lock_test_env`]'s process-wide mutex.
#![allow(unsafe_code)] // env-var mutation (2024-edition set_var/remove_var) guarded by lock_test_env
...
pub struct ScopedCodeSmithHome {
    _env_guard: EnvVarGuard,
    _tempdir: tempfile::TempDir,
}

impl Default for ScopedCodeSmithHome {
    fn default() -> Self {
        Self::new()
    }
}
...
#[track_caller]
pub fn assert_byte_identical(label: &str, a: &str, b: &str) {
