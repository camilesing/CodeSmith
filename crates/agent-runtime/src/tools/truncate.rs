...
//! `~/.codesmith/tool_outputs/<sanitised-id>.txt`. The id is the tool
...
/// Name of the spillover directory under the CodeSmith home.
...
/// startup to keep `~/.codesmith/tool_outputs/` from growing without
/// bound. Mirrors the workspace-snapshot 7-day default.
pub const SPILLOVER_MAX_AGE: Duration = Duration::from_secs(7 * 24 * 60 * 60);

// Test-storage-root override helpers are compiled unconditionally and
// exposed `pub` (not `cfg(test)`-gated) so dependent crates' test builds
// can reach them — dependencies are never built with `cfg(test)` (same
// convention as `crate::test_support` and `crate::artifacts`). They are
// only ever invoked from test contexts; in production the override stays
// `None`.
static TEST_SPILLOVER_ROOT: std::sync::Mutex<Option<PathBuf>> = std::sync::Mutex::new(None);

pub static TEST_SPILLOVER_GUARD: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Resolve `~/.codesmith/tool_outputs/`. Returns `None` if the home
...
pub fn set_test_spillover_root(root: Option<PathBuf>) -> Option<PathBuf> {
...
/// errors are logged and skipped. Mirrors the workspace-snapshot prune
/// step (see the TUI's `session_manager::prune_workspace_snapshots`).
...
/// content to a sibling file under `~/.codesmith/tool_outputs/`,
...
/// `~/.codesmith/sessions/<session-id>/artifacts/`, and the inline tool result
...
/// user's real `~/.codesmith/` directory. This uses explicit test hooks instead
    ...
    F: FnOnce() -> R,
{
    ...
    // artifacts.rs tests.
    let prior_spillover =
        set_test_spillover_root(Some(home.join(".codesmith").join(SPILLOVER_DIR_NAME)));
    let prior_artifacts = crate::artifacts::set_test_artifact_sessions_root(Some(
        home.join(".codesmith").join("sessions"),
    ));
...
mod tests {
    ...
    fn with_test_home_overrides_storage_roots_without_home_resolution() {
        ...
        with_test_home(tmp.path(), || {
            assert_eq!(
                ...
                Some(tmp.path().join(".codesmith").join("tool_outputs").as_path())
            );
            assert_eq!(
    ...
    fn write_spillover_creates_directory_and_writes_file() {
        ...
        with_test_home(tmp.path(), || {
            ...
            // Directory landed under `<HOME>/.codesmith/tool_outputs/`.
            ...
            let components: Vec<&str> = path
                ...
                .collect();
            assert!(
                components.contains(&".codesmith") && components.contains(&"tool_outputs"),
                "spillover path missing expected `.codesmith/tool_outputs/...` segments: {path:?}"
            );
        });
    }
    ...
    #[cfg(unix)]
    #[allow(unsafe_code)] // SAFETY: see the inline note at the `unsafe` block.
    ...
    fn apply_spillover_with_artifact_writes_session_file_and_ref_block() {
        ...
        with_test_home(tmp.path(), || {
            ...
            assert!(
                tmp.path()
                    .join(".codesmith/tool_outputs/call-big.txt")
                    .exists(),
