...
pub const DISCOVERY_ALWAYS_DIRS: &[&str] = &[".codesmith", ".cursor", ".claude", ".agents"];
...
const DISCOVERY_EXCLUDED_SUBDIRS: &[&str] =
    &[".codesmith/snapshots", ".worktrees", ".claude/worktrees"];
...
pub fn path_is_excluded_from_discovery(walk_root: &Path, path: &Path) -> bool {
...
pub fn should_skip_unignored_discovery_entry(walk_root: &Path, path: &Path) -> bool {
    if path == walk_root {
        return false;
    }
