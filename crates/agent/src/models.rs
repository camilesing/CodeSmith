...
/// Context window used only for legacy model IDs that do not name a
/// newer V4 alias and do not carry an explicit `*k` suffix.
pub const LEGACY_MODEL_CONTEXT_WINDOW_TOKENS: u32 = 128_000;
...
pub fn context_window_for_model(model: &str) -> Option<u32> {
    ...
    if lower.contains("deepseek") {
        ...
        return Some(LEGACY_MODEL_CONTEXT_WINDOW_TOKENS);
    }
...
mod tests {
    ...
    fn unknown_legacy_deepseek_models_map_to_128k_context_window() {
        assert_eq!(
            ...
            Some(LEGACY_MODEL_CONTEXT_WINDOW_TOKENS)
        );
        assert_eq!(
