//! Secret storage for CodeSmith API keys.
//!
...
/// a JSON file under `~/.codesmith/secrets/` ([`FileKeyringStore`]), or an
...
pub trait KeyringStore: Send + Sync {
    ...
    /// storage backend is active. Examples: `"file-based (~/.codesmith/secrets/)"`,
...
/// at `<home>/.codesmith/secrets/secrets.json` with Unix file mode `0600`
...
fn default_codesmith_secrets_path() -> Result<PathBuf, SecretsError> {
    if let Ok(value) = std::env::var("CODESMITH_HOME") {
        let trimmed = value.trim();
        if !trimmed.is_empty() {
...
mod tests {
    ...
    #[test]
    fn file_default_path_honors_codesmith_home() {
        let _lock = env_lock();
        clear_known_envs();
        let tmp = tempfile::tempdir().unwrap();
        let custom = tmp.path().join("custom-codesmith");
        let _home = EnvVarGuard::set("HOME", tmp.path());
        let _userprofile = EnvVarGuard::set("USERPROFILE", tmp.path());
        let _codesmith_home = EnvVarGuard::set("CODESMITH_HOME", &custom);
