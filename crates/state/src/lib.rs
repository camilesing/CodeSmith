...
impl StateStore {
    ...
    /// overridable via `CODESMITH_HOME`) is used.
    ...
    fn conn(&self) -> Result<Connection> {
        ...
        // Pin the connection pragmas explicitly rather than relying on
        // libsqlite3-sys build defaults. The bundled SQLite compiles with
        // SQLITE_DEFAULT_FOREIGN_KEYS=1 and rusqlite defaults busy_timeout
        // to 5s — but those are dependency implementation details: a switch
        // to a system SQLite would silently flip FK enforcement OFF and
        // make the schema's ON DELETE CASCADE clauses inert (deleting a
        // thread would orphan its messages/checkpoints forever). WAL
        // additionally lets readers proceed while a writer holds the db
        // (StateStore is Clone-shared across concurrent components).
...
fn default_state_db_path() -> PathBuf {
    ...
    if let Ok(val) = std::env::var("CODESMITH_HOME") {
...
mod tests {
    ...
    fn delete_thread_cascades_to_messages_and_checkpoints() {
        // Regression guard for the schema's ON DELETE CASCADE clauses.
        // libsqlite3-sys (bundled) compiles SQLite with
        // SQLITE_DEFAULT_FOREIGN_KEYS=1, so cascade works today — but that
        // is a build default, not a contract; switching rusqlite to a
        // system SQLite (or a bundled-config change) silently flips FK
        // enforcement OFF and `delete_thread` would orphan rows forever.
        let tmp = tempfile::tempdir().expect("tempdir");
        let store = StateStore::open(Some(tmp.path().join("cascade.db"))).expect("open");
        store
            .upsert_thread(&test_thread("t-cascade"))
            .expect("upsert");

        store
            .append_message("t-cascade", "user", "hello", None)
            .expect("append message");
        store
            .save_checkpoint("t-cascade", "cp1", &serde_json::json!({"k": 1}))
            .expect("checkpoint");

        store.delete_thread("t-cascade").expect("delete");

        // The public listing APIs anchor on threads.current_leaf_id, so they
        // return empty once the thread row is gone regardless — the defect
        // this guards against is orphaned ROWS still in the tables. Count
        // them directly.
        let conn = store.conn().expect("conn");
        let msgs: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM messages WHERE thread_id = 't-cascade'",
                [],
                |r| r.get(0),
            )
        ...
        let cps: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM checkpoints WHERE thread_id = 't-cascade'",
                [],
                |r| r.get(0),
            )
        ...
        assert!(
            msgs == 0 && cps == 0,
            "delete_thread must cascade; {} message rows / {} checkpoint rows remain",
            msgs,
            cps
        );
    }
    ...
    #[test]
    fn connections_enable_wal_journal_mode() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let store = StateStore::open(Some(tmp.path().join("wal.db"))).expect("open");
        let conn = store.conn().expect("conn");
        ...
        assert_eq!(
            mode.to_lowercase(),
            "wal",
            "state db should run in WAL mode for concurrent readers/writers"
        );
    }
