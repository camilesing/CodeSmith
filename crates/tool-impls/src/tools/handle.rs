//! Symbolic handle storage and bounded reads — TUI-side tool + helpers.
//!
//! The portable data types (`SharedHandleStore`, `HandleStore`, `VarHandle`,
//! …) live in `codesmith-agent-runtime::tools::handle`; this module
//! re-exports them so historical `crate::tools::handle::*` paths keep
//! resolving. The `HandleReadTool` `ToolSpec` implementation and the
//! projection helpers stay here because they depend on `spec.rs`, which has
//! not yet migrated to agent-runtime.
...
pub use codesmith_agent_runtime::tools::handle::{
    HandleKey, HandleRecord, HandleStore, HandleValue, SharedHandleStore, VarHandle,
    new_shared_handle_store, truncate_chars,
};
...
fn looks_like_tool_result_ref(raw: &str) -> bool {
    ...
    trimmed.starts_with("art_")
        ...
        || codesmith_agent_runtime::tools::truncate::is_valid_sha256(
