...
pub fn ocr_image_path(image_path: &Path) -> Result<String, ToolError> {
    ...
    Err(ToolError::execution_failed(
        "image_ocr: no local OCR backend is available. On macOS, update to a version with the Vision framework; on Linux/Windows install tesseract and restart codesmith.",
    ))
}
