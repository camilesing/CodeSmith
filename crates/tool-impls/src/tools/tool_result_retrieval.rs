...
//! `~/.codesmith/tool_outputs/<tool-call-id>.txt` by `tools::truncate`. This
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "ref": {
                    "type": "string",
                    "description": "Tool call id, artifact id (`art_<id>`), SHA ref (`sha:<64-hex>`), spillover filename, or absolute path under ~/.codesmith."
                },
                "mode": {
                    "type": "string",
...
/// 6. Absolute paths under the CodeSmith home.
///
...
fn resolve_spillover_reference(reference: &str, session_id: &str) -> Result<PathBuf, ToolError> {
    ...
    // `~/.codesmith/sessions/<sid>/<p>` — so passing the literal
    ...
    let try_path = |candidate: PathBuf, tried: &mut Vec<PathBuf>| -> Option<PathBuf> {
        ...
        // home-level `~/.codesmith/tool_outputs/` dir is engine-only and
