            ...
            Err(_elapsed) => {
                anyhow::bail!(
                    "SSE stream request did not receive response headers after {}s. \
                     `codesmith doctor` can still pass when non-streaming requests work; \
                     on Windows or proxy networks, try `DEEPSEEK_FORCE_HTTP1=1` and rerun `codesmith`.",
...
/// users on `RUST_LOG=codesmith_tui=debug` can see how much of their input
