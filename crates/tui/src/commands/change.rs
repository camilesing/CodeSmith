...
//!
//! Uses the CodeSmith changelog embedded at compile time. With no argument,
    ...
    let latest_section = match section {
        ...
        None => {
            let msg = if let Some(ver) = version {
                let ver = ver.trim();
                if ver.is_empty() {
                    "Could not find a version section in the bundled CodeSmith changelog. \
                     Expected a line starting with `## [`."
                        .to_string()
                } else {
                    format!("Could not find version \"{ver}\" in the bundled CodeSmith changelog.")
                }
            } else {
                "Could not find a version section in the bundled CodeSmith changelog. \
                 Expected a line starting with `## [`."
...
fn inline_changelog_section(section: &str) -> String {
    ...
    format!(
        "{truncated}\n\
\n\
[... {} characters omitted from the bundled CodeSmith changelog]",
