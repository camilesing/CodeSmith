...
/// local cache (`~/.codesmith/cache/skills/`).
...
/// local cache (`~/.codesmith/cache/skills/<name>/`).
///
...
fn needs_approval_message(host: &str) -> String {
    format!(
        "Network policy requires approval for {host}.\n\
         Add it to your allow list with `/network allow {host}` (or set [network].default = \"allow\" in ~/.codesmith/config.toml), then retry."
    )
}
...
fn network_denied_message(host: &str) -> String {
    format!(
        "Network policy denied access to {host}.\n\
         Remove the deny entry from ~/.codesmith/config.toml under [network] or contact your administrator."
    )
}
...
fn registry_fetch_error_hint(err: &anyhow::Error) -> Option<&'static str> {
    ...
    {
        Some(
            "Hint: DNS lookup failed. Check internet/DNS connectivity, or override the registry URL in [skills] of ~/.codesmith/config.toml.",
        )
        ...
        || msg.contains("handshake")
    {
        ...
        Some(
            "Hint: registry URL returned 404. Verify the registry URL in [skills] of ~/.codesmith/config.toml.",
        )
