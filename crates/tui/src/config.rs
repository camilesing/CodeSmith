...
impl Config {
    ...
    fn active_provider_preserves_custom_base_url_model(&self) -> bool {
        let provider = self.api_provider();
        provider_preserves_custom_base_url_model(provider, &self.provider_base_url())
    }
    ...
    pub fn provider_api_key(&self) -> Result<String> {
        ...
        if let Some(id) = self.custom_provider() {
            ...
            if base_url_uses_local_host(&self.provider_base_url()) {
    ...
    /// 2. bare/simple mode (`--bare` / `CODESMITH_SIMPLE` truthy) — closes memory.
    ...
    pub fn memory_enabled(&self) -> bool {
        ...
        if env_flag_truthy_any(&["CODESMITH_SIMPLE"]) {
            return false;
        }
...
/// `CODESMITH_SANDBOX_MODE`. Empty values are skipped so a blank shell export
/// does not erase configured settings.
...
pub fn has_api_key_for(config: &Config, provider: ApiProvider) -> bool {
    ...
    if provider == config.api_provider() && base_url_uses_local_host(&config.provider_base_url()) {
        return true;
    }
...
/// CLI's purview. `Config::provider_api_key`'s explicit-override path
...
mod tests {
    ...
    fn has_api_key_detects_in_memory_override_and_env_var() -> Result<()> {
        ...
        // sources used by `Config::provider_api_key` (in-memory override,
        // env var, config-file slot).
        let _lock = lock_test_env();
        // Explicit in-memory key wins over every other source per
        // `Config::provider_api_key`'s "Path 0" override.
        let cfg = Config {
...
"#,
    ...
    #[test]
    fn provider_api_key_prefers_explicit_in_memory_override() -> Result<()> {
        let _lock = lock_test_env();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
    ...
    #[test]
    fn provider_api_key_prefers_saved_config_over_stale_env() -> Result<()> {
        let _lock = lock_test_env();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
        ...
        assert_eq!(config.provider_api_key()?, "fresh-config-key");
        unsafe {
            env::remove_var("DEEPSEEK_API_KEY");
        }
        Ok(())
    ...
    #[test]
    fn provider_api_key_ignores_sentinel_placeholder() -> Result<()> {
        let _lock = lock_test_env();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
    ...
    fn apply_env_overrides_does_not_copy_api_key_into_config() -> Result<()> {
        ...
        assert_eq!(config.provider_api_key()?, "env-key");
        unsafe {
            env::remove_var("DEEPSEEK_API_KEY");
        }
        Ok(())
    ...
    fn deepseek_provider_defaults_to_beta_endpoint() {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_DEEPSEEK_BASE_URL);
    }
    ...
    fn explicit_deepseek_base_url_overrides_beta_default() {
        ...
        assert_eq!(config.api_provider(), ApiProvider::Deepseek);
        assert_eq!(config.provider_base_url(), "https://api.deepseek.com");
    }
...
"#,
    ...
    fn nvidia_nim_provider_uses_nim_defaults() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_NVIDIA_NIM_BASE_URL);
        Ok(())
    }
    ...
    fn nvidia_nim_env_overrides_provider_and_credentials() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::NvidiaNim);
        assert_eq!(config.provider_api_key()?, "nim-env-key");
        assert_eq!(config.default_model(), DEFAULT_NVIDIA_NIM_MODEL);
        Ok(())
    }
    ...
    fn nvidia_nim_env_accepts_short_nim_base_url_alias() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::NvidiaNim);
        assert_eq!(config.provider_base_url(), "https://short-nim.example/v1");
        Ok(())
    }
    ...
    fn openai_provider_uses_openai_compatible_defaults() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_OPENAI_BASE_URL);
        Ok(())
    }
    ...
    fn xiaomi_mimo_provider_uses_documented_defaults() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_XIAOMI_MIMO_BASE_URL);
        Ok(())
    }
    ...
    fn xiaomi_mimo_env_overrides_provider_base_url_model_and_key() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::XiaomiMimo);
        assert_eq!(config.provider_api_key()?, "mimo-env-key");
        assert_eq!(
            config.provider_base_url(),
    ...
    fn atlascloud_provider_uses_documented_defaults() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_ATLASCLOUD_BASE_URL);
        Ok(())
    }
    ...
    fn atlascloud_env_overrides_provider_base_url_and_model() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Atlascloud);
        assert_eq!(config.provider_api_key()?, "atlascloud-env-key");
        assert_eq!(config.provider_base_url(), "https://api.atlascloud.ai/v1");
    ...
    fn wanjie_ark_provider_uses_documented_defaults() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_WANJIE_ARK_BASE_URL);
        Ok(())
    }
    ...
    fn wanjie_ark_env_overrides_provider_base_url_model_and_key() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::WanjieArk);
        assert_eq!(config.provider_api_key()?, "wanjie-env-key");
        assert_eq!(config.provider_base_url(), "https://wanjie.example/api/v1");
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::WanjieArk);
        assert_eq!(config.provider_api_key()?, "wanjie-table-key");
        assert_eq!(
            config.provider_base_url(),
    ...
    fn anthropic_provider_reads_anthropic_base_url_env() -> Result<()> {
        ...
        assert_eq!(config.provider_api_key()?, "anthropic-env-key");
        assert_eq!(
            config.provider_base_url(),
            "https://token-plan.cn-beijing.maas.aliyuncs.com/apps/anthropic"
        );
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Openai);
        assert_eq!(config.provider_api_key()?, "openai-table-key");
        assert_eq!(
            config.provider_base_url(),
    ...
    // MiniMax-M2.7` forwards the choice via CODESMITH_MODEL (never
    ...
    fn deepseek_model_env_passes_custom_model_through_for_non_deepseek_providers() -> Result<()> {
        ...
        // rewritten to DEFAULT_NOVITA_MODEL.
        {
            ...
            assert_eq!(config.provider_base_url(), DEFAULT_NOVITA_BASE_URL);
    ...
    fn openai_env_overrides_provider_base_url_and_model() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Openai);
        assert_eq!(config.provider_api_key()?, "openai-env-key");
        assert_eq!(
            config.provider_base_url(),
    ...
    fn openai_custom_base_url_with_explicit_model_loads() -> Result<()> {
        ...
        // (a) CODESMITH_MODEL is how the facade forwards the top-level
    ...
    fn openai_env_accepts_facade_base_url_forwarding() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Openai);
        assert_eq!(config.provider_api_key()?, "forwarded-openai-key");
        assert_eq!(
            config.provider_base_url(),
    ...
    fn openrouter_provider_uses_canonical_defaults() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_OPENROUTER_BASE_URL);
        Ok(())
    }
    ...
    fn fireworks_provider_uses_canonical_defaults() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_FIREWORKS_BASE_URL);
        Ok(())
    }
    ...
    fn siliconflow_provider_uses_canonical_defaults() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_SILICONFLOW_BASE_URL);
        assert_eq!(
    ...
    fn sglang_provider_works_without_api_key() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_SGLANG_BASE_URL);
        assert_eq!(config.provider_api_key()?, "");
    ...
    fn ollama_provider_uses_local_defaults_without_api_key() -> Result<()> {
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_OLLAMA_BASE_URL);
        assert_eq!(config.provider_api_key()?, "");
...
"#,
        ...
        assert_eq!(config.provider_base_url(), "http://127.0.0.1:11434/v1");
        Ok(())
    }

    #[test]
    fn provider_base_url_env_scopes_to_self_hosted_providers() -> Result<()> {
        let _lock = lock_test_env();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Ollama);
        assert_eq!(config.provider_base_url(), "http://ollama.remote:11434/v1");
        ...
        assert_eq!(config.provider_base_url(), "http://vllm.remote:8000/v1");
        Ok(())
    }
    ...
    fn vllm_env_resolves_reported_lan_http_endpoint_and_model() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Vllm);
        assert_eq!(config.provider_base_url(), "http://192.168.0.110:8000/v1");
    ...
    fn ollama_env_overrides_base_url_and_model() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Ollama);
        assert_eq!(config.provider_base_url(), "http://ollama.example/v1");
        assert_eq!(config.default_model(), "deepseek-coder-v2:16b");
        Ok(())
    }

    #[test]
    fn openrouter_env_api_key_resolves_via_provider_api_key() -> Result<()> {
        let _lock = lock_test_env();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Openrouter);
        assert_eq!(config.provider_api_key()?, "or-env-key");
        Ok(())
    }

    #[test]
    fn novita_env_api_key_resolves_via_provider_api_key() -> Result<()> {
        let _lock = lock_test_env();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Novita);
        assert_eq!(config.provider_api_key()?, "novita-env-key");
        Ok(())
    }
    ...
    fn siliconflow_env_overrides_key_base_url_and_model() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Siliconflow);
        assert_eq!(config.provider_api_key()?, "sf-env-key");
        assert_eq!(config.provider_base_url(), "https://sf-mirror.example/v1");
        assert_eq!(config.default_model(), "deepseek-v4-flash");
        Ok(())
    }
    ...
    fn siliconflow_cn_base_url_env_normalizes_model_aliases() -> Result<()> {
        ...
        assert_eq!(config.api_provider(), ApiProvider::Siliconflow);
        assert_eq!(config.provider_api_key()?, "sf-env-key");
        assert_eq!(config.provider_base_url(), "https://api.siliconflow.cn/v1");
        assert_eq!(config.default_model(), DEFAULT_SILICONFLOW_MODEL);
        Ok(())
    }
    ...
    fn openrouter_base_url_env_overrides_default() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Openrouter);
        assert_eq!(config.provider_base_url(), "https://or-mirror.example/v1");
        Ok(())
    }
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Openrouter);
        assert_eq!(config.provider_api_key()?, "or-table-key");
        assert_eq!(config.provider_base_url(), "https://or-table.example/v1");
        Ok(())
    }
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Siliconflow);
        assert_eq!(config.provider_api_key()?, "sf-table-key");
        assert_eq!(config.provider_base_url(), DEFAULT_SILICONFLOW_BASE_URL);
...
"#,
        ...
        assert_eq!(config.api_provider(), ApiProvider::Openrouter);
        assert_eq!(config.provider_api_key()?, "or-table-key");
        assert_eq!(config.provider_base_url(), "https://gateway.example.com/v1");
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Novita);
        assert_eq!(config.provider_api_key()?, "novita-table-key");
        assert_eq!(config.provider_base_url(), DEFAULT_NOVITA_BASE_URL);
        Ok(())
    }
...
"#,
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_KIMI_CODE_BASE_URL);
        assert_eq!(config.default_model(), DEFAULT_KIMI_CODE_MODEL);
        assert_eq!(config.provider_api_key()?, "fresh-oauth-token");
...
"#,
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_KIMI_CODE_BASE_URL);
        assert_eq!(config.default_model(), DEFAULT_KIMI_CODE_MODEL);
        assert_eq!(config.provider_api_key()?, "kimi-code-key");
        assert!(has_api_key_for(&config, ApiProvider::Moonshot));
        Ok(())
    }
...
"#,
        ...
        assert_eq!(config.provider_base_url(), DEFAULT_KIMI_CODE_BASE_URL);
        assert_eq!(config.default_model(), DEFAULT_KIMI_CODE_MODEL);
        assert_eq!(config.provider_api_key()?, "kimi-code-env-key");
        assert!(has_api_key_for(&config, ApiProvider::Moonshot));
        Ok(())
    }
    ...
    /// `deepseek-v4-pro`. The user-facing trigger here is the
    /// `CODESMITH_PROVIDER` env var produced by the `codesmith
    /// --provider moonshot` dispatcher.
    #[test]
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Moonshot);
        assert_eq!(config.provider_base_url(), DEFAULT_MOONSHOT_BASE_URL);
        assert_eq!(config.default_model(), DEFAULT_MOONSHOT_MODEL);
        assert_eq!(config.provider_api_key()?, "moonshot-platform-key");
        Ok(())
    }
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::NvidiaNim);
        assert_eq!(config.provider_api_key()?, "nim-table-key");
        assert_eq!(config.provider_base_url(), "https://nim-table.example/v1");
...
"#,
    ...
    fn provider_capability_openai_custom_model_is_chat_completions_without_thinking() {
        ...
        assert_eq!(
            cap.context_window,
            crate::models::LEGACY_MODEL_CONTEXT_WINDOW_TOKENS
        );
    ...
    fn custom_provider_accessors_read_entry_fields() -> Result<()> {
        ...
        assert_eq!(config.provider_api_key()?, "acme-key");
    ...
    fn custom_provider_api_key_falls_back_to_root_then_bails_for_remote() -> Result<()> {
        ...
        assert_eq!(with_root_key.provider_api_key()?, "root-key");
