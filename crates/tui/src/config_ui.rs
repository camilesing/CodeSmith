...
pub fn parse_mode(arg: Option<&str>) -> Result<ConfigUiMode, String> {
    ...
    // of the codesmith-tui navy chrome out of the box. Power users can
...
pub fn build_schema() -> Value {
    ...
    schema["title"] = Value::String("codesmith Config".to_string());
    schema["description"] =
...
impl From<&str> for StatusIndicatorValue {
    fn from(value: &str) -> Self {
        ...
        // so a TOML file with `status_indicator = "none"` resolves to the
        // canonical enum variant.
        match value.trim().to_ascii_lowercase().as_str() {
            "off" | "none" | "hidden" | "false" => Self::Off,
            // Default to dots for "dots", aliases, and anything unknown
            // (we'd rather show the indicator than silently hide it on a
            // typo).
            _ => Self::Dots,
        }
    }
...
mod tests {
    ...
    fn build_document_reflects_cost_currency_from_settings() {
        ...
        let temp_root = std::env::temp_dir().join(format!(
            "codesmith-config-ui-cost-currency-{}-{}",
            std::process::id(),
            nanos
        ));
...
"#,
    ...
    fn build_document_reflects_background_color_from_settings() {
        ...
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let temp_root = std::env::temp_dir().join(format!(
            "codesmith-config-ui-background-color-{}-{}",
            std::process::id(),
            nanos
        ));
        fs::create_dir_all(temp_root.join(".codesmith")).expect("config dir");
...
"##,
    ...
    fn session_only_apply_keeps_runtime_overrides_and_skips_reload() {
        ...
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let temp_root = std::env::temp_dir().join(format!(
            "codesmith-config-ui-session-only-{}-{}",
            std::process::id(),
            nanos
        ));
        fs::create_dir_all(temp_root.join(".codesmith")).expect("config dir");
