//! CLI entry point for the `codesmith-tui` binary (interactive TUI + subcommands).
...
#[command(
    ...
    about = "codesmith/CLI for open-weight coding models",
...
fn run_setup_status(config: &Config, workspace: &Path) -> Result<()> {
    ...
    println!("  · base_url: {}", config.provider_base_url());
    let model = config
        .default_text_model
        .clone()
...
async fn run_doctor(config: &Config, workspace: &Path, config_path_override: Option<&Path>) {
    ...
    let has_api_key = if config.provider_api_key().is_ok() {
...
fn resolve_exec_append_system_prompts(
    ...
    sources.extend(
        args.append_system_prompt
            .iter()
            .enumerate()
            .filter(|&(_index, content)| !content.trim().is_empty())
            .map(|(index, content)| {
...
mod setup_helper_tests {
    ...
    fn env_example_is_trackable_and_every_key_is_wired() {
        ...
        for key in keys {
            ...
            // (`app_env("X")` / `codesmith_env("X")` resolve `CODESMITH_X`).
