...
//! switches are stored separately at ~/.codesmith/tui.toml. See [`TuiPrefs`].
...
impl TuiPrefs {
    ...
    /// the pointed-to config is used instead of `~/.codesmith`).
    pub fn path() -> Result<PathBuf> {
        ...
        Ok(home.join(".codesmith").join("tui.toml"))
    }

    /// Load TUI preferences from `~/.codesmith/tui.toml`.
    ///
    ...
    /// Save TUI preferences to `~/.codesmith/tui.toml`, creating the
    /// `~/.codesmith` directory if needed.
...
pub struct Settings {
    ...
    /// - `"dots"` (default): the 6-frame geometric sequence (`◍ ◉ ◌ ◌ ◉ ◍`).
    ///   Idle frame is a steady `◍`.
...
impl Default for Settings {
    fn default() -> Self {
        Self {
            // Auto-compaction is provider-neutral by default. It prevents
            // 128K/200K providers from reaching hard context errors while
            // explicit `auto_compact = off` remains available for cache-first
            // workflows.
            auto_compact: true,
            calm_mode: false,
            low_motion: false,
...
impl Settings {
    ...
    pub fn set(&mut self, key: &str, value: &str) -> Result<()> {
        match key {
            ...
            "status_indicator" | "indicator" => {
                ...
                if !["dots", "off"].contains(&normalized) {
                    anyhow::bail!(
                        "Failed to update setting: invalid status indicator '{value}'. Expected: dots, off."
                    );
                }
    ...
    pub fn available_settings() -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            (
                "status_indicator",
                "Header status indicator next to effort chip: dots, off",
            ),
            (
                "synchronized_output",
...
mod tests {
    ...
    #[test]
    fn default_settings_enable_provider_neutral_auto_compact() {
    ...
    /// Serialise tests that mutate `CODESMITH_CONFIG_PATH` through this guard
    ...
    fn tui_prefs_path_uses_home_deepseek_subdir_by_default() {
        let _g = config_path_test_guard();
        // Without CODESMITH_CONFIG_PATH the path should end with
        // .codesmith/tui.toml relative to the home directory.
        // We skip this check if home_dir() is unavailable (CI without HOME).
        if let Some(home) = dirs::home_dir() {
            let expected = home.join(".codesmith").join("tui.toml");
            // Only compare when no env override is active.
            if std::env::var("CODESMITH_CONFIG_PATH").is_err() {
