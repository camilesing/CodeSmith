...
mod tests {
    use crate::palette::{
        CATPPUCCIN_MOCHA_UI_THEME, CLAUDE_UI_THEME, DARK_ACCENT_ACTION_RGB,
        DARK_ACCENT_PRIMARY_RGB, DARK_ACCENT_SECONDARY_RGB, DARK_BG_RGB, DARK_TEXT_BODY_RGB,
        DARK_TEXT_MUTED_RGB, DRACULA_UI_THEME, GRAYSCALE_UI_THEME, GRUVBOX_DARK_UI_THEME,
    ...
    #[test]
    fn dark_theme_uses_proposed_palette() {
        // Issue #2012: verify the default dark theme uses proposed tokens.
        let t = UI_THEME;
        assert_eq!(rgb(t.surface_bg), Some(DARK_BG_RGB), "Deep Navy #0A1120");
        assert_eq!(
            rgb(t.text_body),
            Some(DARK_TEXT_BODY_RGB),
            "Whale Ivory #F6F2E8"
        );
        assert_eq!(
            rgb(t.text_muted),
            Some(DARK_TEXT_MUTED_RGB),
            "Mist Gray #A9B4C7"
        );
        assert_eq!(
            rgb(t.accent_primary),
            Some(DARK_ACCENT_PRIMARY_RGB),
            "Signal Gold #F6C453"
        );
        assert_eq!(
            rgb(t.accent_secondary),
            Some(DARK_ACCENT_SECONDARY_RGB),
            "Seafoam #4FD1C5"
        );
        assert_eq!(
            rgb(t.accent_action),
            Some(DARK_ACCENT_ACTION_RGB),
            "Coral Spark #FF7A59"
        );
