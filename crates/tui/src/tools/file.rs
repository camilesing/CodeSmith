...
mod tests {
    ...
    /// Serialises tests that mutate `CODESMITH_CONFIG_PATH` so they don't
    ...
    impl ConfigPathEnvGuard {
        fn capture() -> Self {
            Self {
                prior: std::env::var_os("CODESMITH_CONFIG_PATH"),
            }
        }
    }
    impl Drop for ConfigPathEnvGuard {
        fn drop(&mut self) {
            ...
            match &self.prior {
                Some(v) => unsafe { std::env::set_var("CODESMITH_CONFIG_PATH", v) },
                None => unsafe { std::env::remove_var("CODESMITH_CONFIG_PATH") },
            }
        }
    }
