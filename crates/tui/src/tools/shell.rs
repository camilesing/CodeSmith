...
// concrete `ShellManager` (pty / process management) stays in this module.
...
// The concrete `ShellManager`, `BackgroundShell`, the `ShellManagerHost` bridge,
// and the `ShellApi` / `ShellManagerApi` impls now live in the runtime crate's
// `shell_manager` module (physically migrated in C4-3b.7a.4.c). This TUI module
// retains the crossterm terminal controller, the shared-shell-manager constructor
// that injects it, the `ToolContext`-aware sandbox-decision helper, and the
// `exec_shell` / `shell_wait` / `shell_interact` / `shell_cancel` / `note` tool
// implementations. Historical `crate::tools::shell::X` paths keep resolving via
