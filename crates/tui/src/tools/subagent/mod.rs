...
/// Forge-and-craft terms used as friendly names for sub-agents in the UI —
/// a nod to the CodeSmith name. Tools, techniques, and workshop fixtures.
///
...
/// Return a deterministic smith name for a given agent ID using a hash of
/// the ID string. The same ID always gets the same name — stable across
/// session restarts for persisted agents.
#[must_use]
pub fn smith_name_for_id(id: &str) -> String {
    use std::hash::{Hash, Hasher};
    let mut hasher = std::collections::hash_map::DefaultHasher::new();
    id.hash(&mut hasher);
    let idx = (hasher.finish() as usize) % SMITH_NICKNAMES.len();
    SMITH_NICKNAMES[idx].to_string()
}

/// Assign a unique smith name for an agent ID, avoiding collisions with
/// names already in `active_names`. If the deterministic name is taken,
/// appends a numeric suffix (e.g. "Anvil (2)").
#[must_use]
pub fn assign_unique_smith_name(
    id: &str,
    active_names: &std::collections::HashSet<String>,
) -> String {
    let base = smith_name_for_id(id);
...
#[must_use]
pub fn smith_nickname_for_index(index: usize) -> String {
    let base = SMITH_NICKNAMES[index % SMITH_NICKNAMES.len()];
    if index < SMITH_NICKNAMES.len() {
        base.to_string()
    } else {
        format!("{base} {}", index / SMITH_NICKNAMES.len() + 1)
    }
}
...
impl SubAgent {
    ...
    /// deterministic smith-naming can hash the ID before construction.
    #[allow(clippy::too_many_arguments)]
    fn new(
        id: String,
        agent_type: SubAgentType,
...
impl SubAgentManager {
    ...
    pub(crate) fn spawn_background_with_assignment_options(
        ...
        let nickname = options
            .nickname
            .or_else(|| Some(assign_unique_smith_name(&agent_id, &active_names)));
