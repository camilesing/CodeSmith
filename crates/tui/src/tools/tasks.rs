...
use crate::tools::shell::{ExecShellTool, ShellStatus, ShellWaitTool};
...
impl ToolSpec for TaskGateRunTool {
    ...
    async fn execute(&self, input: Value, context: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        // allowlist (children no longer inherit every parent env var —
        // including API keys), a non-login shell, and process-group kill on
        // timeout (the previous tokio timeout abandoned the child running).
        // The manager API is synchronous and blocks up to the timeout, so
...
mod tests {
    ...
    async fn gate_run_executes_through_shell_manager() {
        ...
        assert_eq!(
            content["gate"]["status"], "passed",
            "gate record: {content}"
        );
        assert!(
            result.content.contains("gate-routed"),
            "gate stdout must be captured"
        );
        // Routing through ShellManager carries sandbox metadata.
        let metadata = result.metadata.expect("metadata");
        assert!(metadata.get("sandboxed").is_some());
    }
    ...
    async fn gate_run_timeout_enforced_by_manager() {
        ...
        let content: Value = serde_json::from_str(&result.content).expect("json content");
        assert_eq!(
            content["gate"]["status"], "timeout",
            "sleep 30 with a 1s budget must time out: {content}"
        );
    }
}
