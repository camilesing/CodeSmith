...
pub use team_discovery::{
    add_member, read_team_config,
    set_member_inactive, write_team_config,
};
pub use team_file::{
    TeamFile, TeamMember, active_teammate_count, active_teammates,
    create_team_file, delete_team_directories, find_member_by_name,
    format_lead_agent_id, read_team_file, remove_member_by_name, sanitize_name, team_config_path, team_lead_name,
};
...
pub use teammate_lifecycle::{
    TeammateRuntime,
    process_inbox_messages, run_teammate_loop,
};
pub use teammate_mailbox::{
    IdleReason, StructuredProtocolMessage, TeammateMessage,
    is_structured_protocol_message, mark_messages_as_read, parse_structured_protocol,
