...
pub struct App {
    ...
    /// Header status-indicator chip mode. One of `"dots"` (default, cycles
    /// geometric ◌ frames keyed off `turn_started_at`), or `"off"` (chip
    /// hidden entirely). Loaded from settings; changed via
    /// `/config status_indicator <dots|off>`.
    ...
    pub workspace_initialization_allowed_at_start: bool,
