...
fn parse_inline_spans(line: &str, base_style: Style, link_style: Style) -> Vec<InlineToken> {
    ...
    while !rest.is_empty() {
            ...
            && let Some(end) = rest[1..].find('*')
        {
            ...
            // identifier like `codesmith_tui`, not italic markup).
...
mod tests {
    ...
    fn underscores_inside_identifiers_render_as_literal_text() {
        ...
        // markdown parser ate the underscore in `codesmith_tui` because
        ...
        let cases = [
            "crate codesmith_tui handles approvals",
    ...
    fn wrapped_osc_8_url_chunks_keep_full_link_target() {
        let url = "https://raw.githubusercontent.com/camilesing/codesmith-skills/main/index.json";
        ...
        assert!(
            !joined.contains(
                "\x1b]8;;https://raw.githubusercontent.com/camilesing/codesmith-skills/main/inde\x1b\\"
            ),
    ...
    fn table_pipes_inside_inline_code_stay_in_the_cell() {
        let src = "| Check | Result |\n\
                   |---|---|\n\
                   | `strings ~/.cargo/bin/codesmith-tui | grep -c \"legacy marker\"` | 0 matches |\n";
        let parsed = parse(src);
        ...
        assert_eq!(
            rows[1],
            &vec![
                "`strings ~/.cargo/bin/codesmith-tui | grep -c \"legacy marker\"`".to_string(),
                "0 matches".to_string(),
            ]
        );
        ...
        let data_line = rendered_lines
            .iter()
            .find(|line| line.contains("strings ~/.cargo/bin/codesmith-tui"))
