...
async fn run_event_loop(
    ...
    if !app.balance_initiated && should_fetch_deepseek_balance(app) {
        ...
        let api_key = config.provider_api_key().unwrap_or_default();
    ...
    loop {
            ...
            while let Ok(event) = rx.try_recv() {
                ...
                match event {
                    ...
                    EngineEvent::TurnComplete {
                        ...
                        {
                            let mut context =
                                app.base_hook_context().with_cost(app.session.session_cost);
        ...
        if event::poll(poll_timeout)? {
            ...
            match key.code {
                ...
                // no indicator and no warning. The Alt-prefixed shortcuts mirror
