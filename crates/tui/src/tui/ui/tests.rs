...
impl ConfigPathEnvGuard {
    fn new() -> Self {
        ...
        let previous = std::env::var_os("CODESMITH_CONFIG_PATH");
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            if let Some(previous) = self.previous.take() {
                std::env::set_var("CODESMITH_CONFIG_PATH", previous);
            } else {
...
fn active_tool_status_label_strips_shell_wrappers_from_ci_polling() {
    ...
    active.push_tool(
        "exec-1",
        HistoryCell::Tool(ToolCell::Exec(ExecCell {
            command: "cd /tmp/repo && sleep 15 && gh pr checks 1611 --repo camilesing/CodeSmith"
                .to_string(),
            status: ToolStatus::Running,
            output: None,
            started_at: app.turn_started_at,
