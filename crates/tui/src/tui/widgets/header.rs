...
/// Geometric status-indicator frames.
...
///
/// `mode` accepts the canonical names `"dots"`, `"off"` (any other value
/// is treated as `"dots"`). `"off"` returns `None` so the caller can hide
/// the chip outright.
#[must_use]
pub fn header_status_indicator_frame(
    ...
    let frames: &[&str] = match mode.trim().to_ascii_lowercase().as_str() {
        ...
        // "dots" + aliases + unknown → dots (intentional default).
...
impl<'a> HeaderWidget<'a> {
    ...
    fn status_variant(
        ...
        // Status indicator chip (dots ◌/◉ depending on `status_indicator`
        // setting). Sits immediately before the effort chip so the layout
        // reads e.g. `◍  ◆ max`.
...
mod tests {
    ...
    fn wide_header_shows_plain_mode_and_single_metadata_cluster() {
        ...
        assert!(rendered.contains("codesmith-tui"));
    ...
    #[test]
    fn dots_indicator_idle_frame_is_first_geometric_glyph() {
    ...
    fn unknown_indicator_mode_defaults_to_dots() {
        // We'd rather show the dots on a typo than silently hide the
        // chip — matches `StatusIndicatorValue::from(&str)`.
        let frame = super::header_status_indicator_frame(None, "dost-typo");
        assert_eq!(frame, Some("\u{25CD}"));
    }

    #[test]
    fn header_renders_indicator_chip_next_to_effort_label() {
        let rendered = render_header(
            HeaderData::new(
                AppMode::Agent,
                "deepseek-v4-pro",
            ...
            .with_status_indicator(Some("\u{25CD}")),
            72,
        );
        assert!(
            rendered.contains("\u{25CD}"),
            "expected indicator chip in header, got: {rendered}"
        );
        assert!(
            rendered.contains("max"),
            "expected effort chip preserved, got: {rendered}"
        );
        // Indicator appears before "max" — sanity-check ordering by index.
        let indicator_idx = rendered.find("\u{25CD}").expect("indicator present");
        let max_idx = rendered.find("max").expect("max present");
        assert!(
            indicator_idx < max_idx,
            "expected indicator to render before effort label, got: {rendered}"
        );
    }

    #[test]
    fn header_hides_indicator_chip_when_status_indicator_off() {
        let rendered = render_header(
            HeaderData::new(
                AppMode::Agent,
                "deepseek-v4-pro",
        ...
        assert!(!rendered.contains("\u{25CD}"));
