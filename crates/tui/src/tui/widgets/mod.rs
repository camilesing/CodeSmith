...
impl Renderable for ChatWidget {
    fn render(&self, _area: Rect, buf: &mut Buffer) {
        ...
        // Repaint the full chat area with the codesmith-ink background each
...
mod tests {
    ...
    fn slash_completion_hints_exclude_set_and_deepseek_commands() {
        ...
        assert!(!hints.iter().any(|hint| hint.name == "/codesmith"));
    }
    ...
    fn empty_state_shows_startup_context() {
        let mut app = create_test_app();
        app.workspace = PathBuf::from("/tmp/codesmith-test-workspace");
        ...
        assert!(rendered.contains(&format!(">_ codesmith (v{})", env!("CARGO_PKG_VERSION"))));
        assert!(rendered.contains("model: deepseek-v4-pro  /model to switch"));
        assert!(rendered.contains("directory: /tmp/codesmith-test-workspace"));
    }
    ...
    ///
    /// Run with: `cargo test -p codesmith-tui --release bench_transcript_scroll
    /// -- --ignored --nocapture`
