...
- builds Linux x64 release assets from `v*` tags, creates the CNB release, and
  uploads `codesmith-linux-x64`, `codesmith-tui-linux-x64`, and
...
- `CODESMITH_REPO_BRANCH`: branch or tag to deploy, for example `main`

Optional:

- `CODESMITH_REPO_URL`: defaults to the CNB mirror URL
- `LIGHTHOUSE_SSH_PORT`: defaults to `22`

The server side should already have `/opt/whalebro/codesmith`,
`/etc/codesmith/runtime.env`, `/etc/codesmith/feishu-bridge.env`, and the
`codesmith-runtime` / `codesmith-feishu-bridge` systemd services (the bridge
side is documented in
...
- Do not store Feishu App Secret or DeepSeek API keys in CNB. They belong in
  `/etc/codesmith/*.env` on Lighthouse.
