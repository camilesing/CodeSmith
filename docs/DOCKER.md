...
`ghcr.io/camilesing/codesmith:latest` and the semver tags are conservative runtime
images:
...
CodeSmith does not automatically execute `.codesmith/setup.sh`. If you keep
that file as a local project recipe, run it explicitly. For shared team setup, prefer a committed project script or
...
|-----------------------|----------|--------------------------------------------------|
| `CODESMITH_API_KEY`   | yes      | Provider API key (DeepSeek by default)                                 |
