...
CodeSmith ships matched `codesmith` and `codesmith-tui` prebuilt binaries for
...
> from `npm i -g codesmith`. v0.8.8 publishes both `codesmith-linux-arm64`
> and `codesmith-tui-linux-arm64`, so a plain `npm i -g codesmith` works
...
`https://github.com/camilesing/CodeSmith/releases` and the npm package named
`codesmith`. Do not install release assets from look-alike repositories,
archives, or search-result mirrors unless you deliberately trust that mirror.

Every GitHub release includes `codesmith-artifacts-sha256.txt`. If you download
...
On macOS, use `shasum -a 256 -c codesmith-artifacts-sha256.txt` instead of
`sha256sum`.
...
- the filename, for example `codesmith-linux-x64`
...
release, verifies a SHA-256 manifest, and exposes both `codesmith` and
`codesmith-tui` on your `PATH`.

Useful environment variables:

| Variable                            | Purpose                                                                                |
| ----------------------------------- | -------------------------------------------------------------------------------------- |
| `CODESMITH_VERSION`              | Pin which release the wrapper downloads (defaults to `codesmithBinaryVersion`)          |
...
> [!WARNING]
> **Not ready yet.** The `cargo install codesmith-cli` / `codesmith-tui`
> commands below pull crates that are not published yet. The mirror setup
> itself still applies to [building from source](#7-build-from-source).
...
CNB deploy templates live in
[deploy/tencent-lighthouse/cnb](../deploy/tencent-lighthouse/cnb/README.md); the
Feishu/Lark phone bridge lives in
[integrations/feishu-bridge](../integrations/feishu-bridge/README.md).
...
Nix builds `codesmith-tui` and then starts the `codesmith` dispatcher. Pass
...
[Releases page](https://github.com/camilesing/CodeSmith/releases) and drop them
...
The `codesmith` package is listed in Scoop's main bucket:
...
`target/aarch64-unknown-linux-gnu/release/codesmith` and
`target/aarch64-unknown-linux-gnu/release/codesmith-tui`. Copy the matched pair
...
Both binaries appear in `target\release\codesmith.exe` and
`target\release\codesmith-tui.exe`.
...
binaries. Either upgrade (`npm i -g codesmith@latest`) or use
`cargo install` per [Section 4](#4-install-via-cargo-any-tier-1-rust-target).

### `MISSING_COMPANION_BINARY` at runtime

The dispatcher (`codesmith`) requires the TUI runtime (`codesmith-tui`) to be on
...
The mirror directory must contain `codesmith-artifacts-sha256.txt` and the
...
### Wrapper installs but `codesmith` isn't found
...
3. **Use `npm install -g codesmith` instead** — the npm package ships
   prebuilt binaries and skips the Cargo build entirely
   ([Section 3](#3-install-via-npm-recommended)).
4. **Use `cargo install codesmith-cli --locked`** from crates.io — this
...
If `codesmith` waits several seconds and prints `connect ETIMEDOUT` or
...
2. Mirror the release assets internally and set `CODESMITH_RELEASE_BASE_URL`:
   ...
   The directory must contain `codesmith-artifacts-sha256.txt` and the platform
...
4. Download both `codesmith` and `codesmith-tui` manually from the
   [Releases page](https://github.com/camilesing/CodeSmith/releases), place them
