# Rebrand: DeepSeek TUI → CodeSmith

Starting with **v0.8.41**, this project ships under a new name: `codesmith`.
...
|---|---|---|
| CLI dispatcher binary | `deepseek` | `codesmith` |
| TUI runtime binary | `deepseek-tui` | `codesmith-tui` |
| npm wrapper package | `deepseek-tui` | `codesmith` |
| Crates.io crates | `deepseek-tui-cli` / `deepseek-tui` / `deepseek-*` | `codesmith-cli` / `codesmith-tui` / `codesmith-*` |
| Release assets | `deepseek-<platform>` / `deepseek-tui-<platform>` | `codesmith-<platform>` / `codesmith-tui-<platform>` |
| Checksum manifest | `deepseek-artifacts-sha256.txt` | `codesmith-artifacts-sha256.txt` |
...
- **Config directory**: `~/.deepseek/`. Renaming this would invalidate
  ...
  and audit log.
- **GitHub repository URL**: `https://github.com/Hmbown/CodeSmith`.
...
- **Docker image**: `ghcr.io/hmbown/codesmith`.
...
- A `deepseek` binary that prints a one-line warning to stderr and forwards
  argv to `codesmith`.
- A `deepseek-tui` binary that does the same for `codesmith-tui`.
...
`v0.8.41` Releases attach **both** the canonical `codesmith-*` /
`codesmith-tui-*` assets and the legacy `deepseek-*` / `deepseek-tui-*`
...
an alias of `codesmith-artifacts-sha256.txt` so v0.8.40's hardcoded lookup
still verifies.

## Why the name change

CodeSmith is a shorter, terminal-friendlier handle for the same terminal
...
to CodeSmith; the official DeepSeek provider, model IDs, env vars, and
...
<https://github.com/Hmbown/CodeSmith/issues> and include:

- The output of `codesmith --version` (or `deepseek --version` if you're
  still on the shim).
