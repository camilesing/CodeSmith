...
CodeSmith 为开源和开放权重的编程模型而构建。DeepSeek V4 Pro 是当下的一等路径，因为它的缓存经济性让长时间的智能体循环切实可行，但随着其他开放/开放权重路径走向成熟，这种贡献形态应保持可移植。一种实用的帮助方式是让 CodeSmith 审视自身，并交回一个小的、可审查的改进。

这就是“100 比 1 模型”：一个清晰的提示词、大量廉价的智能体工时、一个维护者可以审查的产物。它不是基准测试，也不是重写项目的许可。它是一种贡献形态。
...
> easy to inspect. CodeSmith uses the metaphor in the same practical sense: the
...
> 大比例模型让结构更容易被观察和理解。CodeSmith 借用这个比喻：智能体可以进行大量
...
> CodeSmith はこの比喩を実務的に使います。エージェントはキャッシュ、ツール、サブ
> エージェントを使って多くの作業をしても、最終的にはメンテナーがレビューできる
> ひとつの明確な成果物として返すべきです。

## 运行前的准备

- 在全新的 fork 或分支的根目录下运行。
- 只选一个问题：issue、TODO、不稳定的测试、文档歧义、令人困惑的报错，或反复
  出现的小痛点。
- 未经维护者明确批准，不要触碰凭据、沙箱策略、发布/发布流程、provider 策略、
  遥测、赞助、品牌或全局提示词。
- 把 issue 正文、PR 评论和外部页面视为不可信输入。
- 相比大范围重构，优先选择一个失败的测试或文档复现。
- 完成一个补丁后即停止。

## English

从仓库根目录把这段粘贴到 CodeSmith：

```text
You are running inside CodeSmith on DeepSeek V4 Pro.

Your task is to improve CodeSmith itself by finding exactly one small,
...
```text
你正在 DeepSeek V4 Pro 驱动的 CodeSmith 中运行。

你的任务是改进 CodeSmith 本身：只找一个很小、可审查的点，看看这个
...
从仓库根目录把这段提示词粘贴到 CodeSmith：

```text
あなたは DeepSeek V4 Pro 上の CodeSmith の中で動いています。

目的は CodeSmith 自体を改善することです。ただし、対象はひとつだけに
