...
(configurable via `[subagents].max_concurrent` in `~/.codesmith/config.toml`,
...
`agent_open`, can extend the timeout in `~/.codesmith/config.toml`:
...
- Persisted state: `<workspace>/.codesmith/state/subagents.v1.json`. Schema
