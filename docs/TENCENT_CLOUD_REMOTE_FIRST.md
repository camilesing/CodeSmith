...
This is the opinionated Tencent-native teaching path for codesmith users
who want an always-on agent workspace, a phone control surface, and a stack
that works well from mainland China.

It complements the local install path. If you only want to use `codesmith` on a
laptop, start with the README quickstart. If you want "CodeSmith as a remote
...
- **Lighthouse** is the private always-on host. It owns `/opt/whalebro`,
  systemd, Rust/Node installs, and the `codesmith serve --http` runtime.
...
3. Update `/opt/whalebro/codesmith`.
...
Use this sequence when explaining codesmith to a new remote-first user:

1. **Local mental model:** `codesmith` is the dispatcher, `codesmith-tui` is the
   companion runtime, and both binaries matter.
2. **Agent safety:** Plan/Agent/YOLO are separate from approval mode and
   sandboxing.
3. **Remote runtime:** `codesmith serve --http` is a localhost runtime API, not
   a public web app.
