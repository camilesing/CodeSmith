...
always-on codesmith host controlled from Feishu/Lark on a phone.
...
Install Rust 1.88+ for the `codesmith` user, then build both shipped binaries:
...
7. Reboot the instance, then confirm `systemctl status codesmith-runtime` and
   `systemctl status codesmith-feishu-bridge` return to active.

## Operational Notes

- Bind `codesmith serve --http` to `127.0.0.1`.
- Keep the Lighthouse firewall focused on SSH for this setup.
- Use SSH key auth.
- Use `tmux` for emergency terminal work from Blink/Termius.
- Keep `/opt/whalebro/codesmith` on a personal branch while working from the
  phone.
