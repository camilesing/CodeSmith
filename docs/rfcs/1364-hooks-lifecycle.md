...
CodeSmith already has lifecycle hooks and MCP support, but the current hook
...
command = "~/.codesmith/hooks/inject-memory.sh"
timeout_secs = 2
...
> and the `CODESMITH_SESSION_ID` env var now carry an **ephemeral**
...
> separately as `CODESMITH_THREAD_ID` and as a `thread_id` field in structured
> payloads (`message_submit`, `pre_compact`). Hook authors who need
> cross-restart correlation must use `CODESMITH_THREAD_ID`, not
> `CODESMITH_SESSION_ID`.
...
- `CODESMITH_SESSION_ID` now carries an ephemeral telemetry id (behavior
  change: it no longer correlates across restarts). `CODESMITH_THREAD_ID` is the
