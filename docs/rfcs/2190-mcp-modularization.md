...
### 1.1 `codesmith-mcp` crate (`crates/mcp/`)
...
- **MCP stdio server** — a minimal MCP server that exposes CodeSmith's own tools
  over stdio for external MCP clients. Used by the `codesmith mcp` CLI subcommand.
...
- The `codesmith` CLI (non-interactive mode)
...
**Dependencies:** `serde`, `serde_json`, `codesmith-protocol` (for tool schema)
...
2. Re-export from `codesmith-mcp` for backward compatibility
3. Update `Cargo.toml` in `codesmith-mcp` to depend on `codesmith-mcp-protocol`
...
3. Update `codesmith-tui` to depend on `codesmith-mcp-client`
4. Update `codesmith-cli` to depend on `codesmith-mcp-server`
5. Deprecate `codesmith-mcp` crate (re-exports from new crates)
...
2. Encrypted file in `~/.codesmith/mcp-credentials/` (fallback)
...
| Breaking internal imports | Phase 2 carries `codesmith-mcp` deprecation shim for one release |
