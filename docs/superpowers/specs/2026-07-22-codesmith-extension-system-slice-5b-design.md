...
- **命名辨析:** `crates/tui/src/workspace_trust.rs`(外部路径 allowlist,`~/.deepseek/workspace-trust.json`,非 persisted workspace trust)——**勿混**;本切片 trust gate 用 `config::is_workspace_trusted`(persisted `[projects]` trust),已验证。
