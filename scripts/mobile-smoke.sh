...
# Launches the compiled codesmith-tui binary on loopback ports and verifies
# the mobile control page, auth, API routes, and binding behaviour through
# real HTTP requests.
#
# Usage:  ./scripts/mobile-smoke.sh
# Requires: curl, a built binary at target/release/codesmith-tui
...
BINARY="${BINARY:-${REPO_ROOT}/target/release/codesmith-tui}"
PASS=0
FAIL=0
SERVER_PID=""
...
if [[ ! -x "$BINARY" ]]; then
    log "Binary not found; building codesmith-tui in release mode..."
    cargo build -p codesmith-tui --release --locked
fi
...
assert_body_contains GET "/mobile?token=${TOKEN}" "" "CodeSmith Mobile"
...
assert_body_contains GET "/mobile" "" "CodeSmith Mobile"
