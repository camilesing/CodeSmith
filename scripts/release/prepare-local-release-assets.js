...
const {
  ...
  CHECKSUM_MANIFEST,
  detectBinaryNames,
} = require("../../npm/codesmith/scripts/artifacts");

const WINDOWS_LAUNCHER = "codesmith.bat";
const WINDOWS_CLI_ASSET = "codesmith-windows-x64.exe";
...
async function main() {
  ...
  const assets = [
    {
      source: path.join(buildDir, isWindows ? "codesmith.exe" : "codesmith"),
      target: codesmith,
    },
    {
      source: path.join(buildDir, isWindows ? "codesmith-tui.exe" : "codesmith-tui"),
      target: tui,
    },
  ];
  ...
  if (prepareAllAssets) {
    for (const assetName of allAssetNames()) {
      ...
      assets.push({
        source: assetName.startsWith("codesmith-tui")
          ? path.join(buildDir, isWindows ? "codesmith-tui.exe" : "codesmith-tui")
          : path.join(buildDir, isWindows ? "codesmith.exe" : "codesmith"),
        target: assetName,
      });
    }
  }
  ...
  if (assets.some((asset) => asset.target === WINDOWS_CLI_ASSET)) {
    const batContent = [
      ...
      "set NO_ANIMATIONS=1",
      'if "%ERRORLEVEL%"=="0" (',
      '    wt --title CodeSmith cmd /k "%~dp0codesmith-windows-x64.exe"',
      ") else (",
      '    "%~dp0codesmith-windows-x64.exe"',
      ")",
      "",
    ].join("\r\n");
