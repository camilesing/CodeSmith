...
Community site for [CodeSmith](https://github.com/Hmbown/CodeSmith) — lives at **codesmith.net**.
...
| `GITHUB_REPO`       | Defaults to `Hmbown/CodeSmith`                    | optional             |
...
You already own `codesmith.net` on Cloudflare and have a Workers Paid plan. The deploy is two steps:
...
3. **Point the domain:** in the Cloudflare dashboard, add a Worker route for `codesmith.net/*` → the deployed Worker (currently named `deepseek-tui-web` unless the Worker is renamed during deploy).

The first cron run happens within 6 hours; you can also kick it manually:

```bash
curl -H "x-cron-secret: $CRON_SECRET" "https://codesmith.net/api/cron?task=curate"
```
