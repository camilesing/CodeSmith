...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: isZh ? "路线图 · CodeSmith" : "Roadmap · CodeSmith",
    description: isZh
      ? "已确认、正在评估和已排除的功能规划。"
      : "What's confirmed, what's being weighed, what's been ruled out for CodeSmith.",
  };
}
...
const tracksEn = [
  {
    ...
    items: [
      ...
      { title: "Durable sessions + tasks", note: "Save, resume, rollback; background task queue with replayable timelines under ~/.codesmith/tasks/" },
      { title: "Bidirectional MCP", note: "Consume tools from external servers; expose as server via `codesmith mcp`; ~/.codesmith/mcp.json" },
      { title: "Skills + unified slash palette", note: "~/.codesmith/skills/ auto-loading; /help, /mode, /status, /config, /trust, /feedback" },
...
const tracksZh = [
  {
    ...
    items: [
      ...
      { title: "持久化会话 + 后台任务", note: "保存、恢复、回滚；后台任务队列，可回放时间线，位于 ~/.codesmith/tasks/" },
      { title: "双向 MCP 协议", note: "消费外部服务器工具；通过 `codesmith mcp` 暴露为服务器；~/.codesmith/mcp.json" },
      { title: "技能 + 统一命令面板", note: "~/.codesmith/skills/ 自动加载；/help、/mode、/status、/config、/trust、/feedback" },
        ...
        <>
          <section className="mx-auto max-w-[1400px] px-6 pt-12 pb-8">
            ...
            <p className="mt-5 max-w-3xl text-ink-soft text-lg leading-[1.9] tracking-wide">
              ...
              <Link href="https://github.com/Hmbown/CodeSmith/discussions/new?category=ideas" className="body-link">
                Discussions
              </Link>{" "}
              中讨论。
            </p>
          ...
          <section className="bg-ink text-paper">
            <div className="mx-auto max-w-[1400px] px-6 py-12 grid lg:grid-cols-12 gap-6 items-center">
              ...
              <div className="lg:col-span-4 flex flex-col gap-3">
                <Link
                  href="https://github.com/Hmbown/CodeSmith/discussions/new?category=ideas"
                ...
                <Link
                  href="https://github.com/Hmbown/CodeSmith/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22"
