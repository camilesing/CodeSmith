...
const LINK_TARGETS: { url: string; label: string }[] = [
  { url: "https://github.com/Hmbown/CodeSmith", label: "Main repo" },
  { url: "https://github.com/Hmbown/CodeSmith/issues", label: "Issues" },
  { url: "https://github.com/Hmbown/CodeSmith/pulls", label: "Pull Requests" },
  { url: "https://github.com/Hmbown/CodeSmith/discussions", label: "Discussions" },
  { url: "https://github.com/Hmbown/CodeSmith/releases", label: "Releases" },
  { url: "https://github.com/Hmbown/CodeSmith/blob/main/LICENSE", label: "License file" },
  { url: "https://github.com/Hmbown/CodeSmith/blob/main/CODE_OF_CONDUCT.md", label: "Code of Conduct" },
  { url: "https://github.com/Hmbown/CodeSmith/blob/main/SECURITY.md", label: "Security policy" },
  { url: "https://github.com/Hmbown/CodeSmith/blob/main/CONTRIBUTING.md", label: "Contributing guide" },
  { url: "https://github.com/Hmbown/CodeSmith/blob/main/.github/PULL_REQUEST_TEMPLATE.md", label: "PR template" },
...
const SEMANTIC_DRIFT_PROMPT = `You are reviewing copy on a community website (codesmith.net) for the open-source CodeSmith project.
...
export async function runSemanticDrift(env: WatchEnv): Promise<{ ok: boolean; drafted: number; reason?: string }> {
  ...
  const ghHeaders: Record<string, string> = {
    ...
    "User-Agent": "codesmith-web-semantic-drift",
  };
  ...
  const [changelog, commits, homepageHtml, docsHtml] = await Promise.all([
    fetch("https://raw.githubusercontent.com/Hmbown/CodeSmith/main/CHANGELOG.md", { headers: ghHeaders }).then((r) => r.ok ? r.text() : "").catch(() => ""),
    fetch("https://api.github.com/repos/Hmbown/CodeSmith/commits?per_page=30", { headers: ghHeaders }).then((r) => r.ok ? r.json() as Promise<{ commit: { message: string }; sha: string }[]> : []).catch(() => []),
...
${docsText}`;
  ...
  for (const d of drifts) {
    ...
    const draft: AgentDraft = {
      id,
      type: "triage",
      targetUrl: `https://codesmith.net/en/${d.page === "homepage" ? "" : d.page}`,
      bodyEn: body,
      bodyZh: body,
      generatedAt: new Date().toISOString(),
      posted: false,
