...
const RAW_BASE = "https://raw.githubusercontent.com/Hmbown/CodeSmith/main";
...
async function fetchListing(dir: string, ghToken?: string): Promise<string[] | null> {
  ...
  const url = `https://api.github.com/repos/Hmbown/CodeSmith/contents/${dir}?ref=main`;
  const headers: Record<string, string> = {
...
async function fetchLatestRelease(ghToken?: string): Promise<string | null> {
  ...
  if (ghToken) headers["Authorization"] = `Bearer ${ghToken}`;
  try {
    const r = await fetch("https://api.github.com/repos/Hmbown/CodeSmith/releases/latest", { headers });
    if (!r.ok) return null;
