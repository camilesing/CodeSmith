...
describe("lastPageFromLink", () => {
  ...
  it("extracts page from Link header with last rel", () => {
    const link =
      '<https://api.github.com/repos/Hmbown/CodeSmith/issues?page=5>; rel="last"';
    expect(lastPageFromLink(link)).toBe(5);
  });

  it("extracts page from multi-part Link header", () => {
    const link = [
      '<https://api.github.com/repos/Hmbown/CodeSmith/issues?page=1>; rel="prev"',
      '<https://api.github.com/repos/Hmbown/CodeSmith/issues?page=3>; rel="last"',
    ].join(", ");
